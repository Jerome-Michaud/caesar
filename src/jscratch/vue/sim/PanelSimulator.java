...
public class PanelSimulator extends JPanel implements MouseWheelListener, MouseMotionListener, MouseListener, ObservablePanelSimulator, ObserverSimulator {
	...
	public PanelSimulator(Simulator simulator) {
		...
		robotRenderer = new RobotRenderer(simulator.getRobot());
	...
	/**
	 * Renvoie le renderer du robot
	 * @return Le render du robot
	 */
	...
	public void mouseDragged(MouseEvent event) {
		...
		simulator.getRobotController().deplacerRobot(event.getX() - (int) ((d.getWidth() - map.getTailleX()) / 2), event.getY() - (int) ((d.getHeight() - map.getTailleY()) / 2));
		repaint();
		notifyObserver();
	}
	...
	public void paintComponent(Graphics g){
		...
		mapRenderer.render(gScreen);
		robotRenderer.render(gScreen);

		Dimension d = this.getSize();
		this.setPreferredSize(new Dimension(map.getTailleX(), map.getTailleY()));

		g.drawImage(screen, (int) ((d.getWidth() - map.getTailleX()) / 2), (int) ((d.getHeight() - map.getTailleY()) / 2), map.getTailleX(), map.getTailleY(), null);
	}
