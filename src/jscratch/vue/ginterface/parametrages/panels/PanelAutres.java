...
public final class PanelAutres extends PanelParametrage {
	...
	private PanelAutres() {
		...
		if (os.contains("windows"))
			this.table.addProperty("Chemin vers le compilateur",  new File(VariableProperties.CHEMIN_ACCES_NBC_WIN));
		if (os.contains("mac"))
			this.table.addProperty("Chemin vers le compilateur",  new File(VariableProperties.CHEMIN_ACCES_NBC_MAC));
		if (os.contains("linux"))
			this.table.addProperty("Chemin vers le compilateur",  new File(VariableProperties.CHEMIN_ACCES_NBC_LINUX));
	...
	public void update() {
		...
		String os = System.getProperty("os.name").toLowerCase();
		if (os.contains("windows"))
			p.set("user.chemin.compilateur.win", modele.getPropertyValue(0).toString());
		if (os.contains("mac"))
			p.set("user.chemin.compilateur.mac", modele.getPropertyValue(0).toString());
		if (os.contains("linux"))
			p.set("user.chemin.compilateur.linux", modele.getPropertyValue(0).toString());
